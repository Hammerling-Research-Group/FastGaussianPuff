...
def test_multisource_setup():
  p = parser('./parser_inputs/multi.in')
  p.run_exp()
  ...
  start_time = pd.to_datetime("2022-04-08 10:22:21").floor('min')
  end_time = pd.to_datetime("2022-04-08 13:22:22").floor('min')
  ...
  parser_ch4 = pd.read_csv('./parser_out/04-08-22_10:22_exp_0.csv')
  for i in range(0, len(sensors)):
    diff = np.abs(parser_ch4[names[i]].values - ch4[:,i])
    assert np.linalg.norm(diff) < 1e-3
